...
def load_image(
    ...
    if is_gray:
        image = np.array(imageio.imread(image_path, mode='L')).astype(np.float32)
    else:
        image = np.array(imageio.imread(image_path, mode='RGB')).astype(np.float32)
